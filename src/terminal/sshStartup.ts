...
(
  ...
  while true; do
    command ssh -o ServerAliveInterval=15 -o ServerAliveCountMax=3 ${host}
    sikemux_ssh_status=$?
    stty sane 2>/dev/null || true
    printf '\\033[0m\\033[?25h\\033[?1l\\033>\\033[?2004l\\033[?1000l\\033[?1002l\\033[?1003l\\033[?1006l'

    if [ "$sikemux_ssh_status" -eq 0 ]; then
      break
    fi
    if [ "$sikemux_ssh_status" -eq 130 ]; then
      printf '\\r\\nSSH reconnect cancelled. Back at your local shell.\\r\\n'
      break
    fi
    if [ "$sikemux_ssh_retries" -ge ${SSH_MAX_RETRIES} ]; then
      printf '\\r\\nSSH could not reconnect after ${SSH_MAX_RETRIES} retries. Back at your local shell.\\r\\n'
      break
    fi

    sikemux_ssh_retries=$((sikemux_ssh_retries + 1))

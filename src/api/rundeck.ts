...
async function rndInvoke<T>(
  ...
  try {
    ...
    const isAuth =
      cat === "rundeck-auth" ||
      cat === "rundeck-unconfigured" ||
      (cat === "rundeck-http" && (err?.status === 401 || err?.status === 403));
    if (isAuth) {
      emit({
...
export type PushAction =
  ...
  | "will-not-push-no-repo"
  | "will-not-push-detached";
...
export const rundeckApi = {
  ...
  login: (req: RundeckLoginRequest) =>
    rndInvoke<RundeckLoginResult>("rnd_login", { req }),
  ...
  branchesMatrix: (envs: RundeckEnvSpec[]) =>
    rndInvoke<MatrixResult>("rnd_branches_matrix", { envs }),
  resolveJob: (project: string, service: string) =>
    rndInvoke<RundeckJob>("rnd_resolve_job", { project, service }),
  ...
  execution: (executionId: number) =>
    rndInvoke<RundeckExecution>("rnd_execution", { executionId }),
  executionState: (executionId: number) =>
    rndInvoke<RundeckWorkflowState>("rnd_execution_state", { executionId }),
  run: (
    project: string,
    service: string,
    branch: string,
  ...
  abort: (executionId: number) =>
    rndInvoke<AbortResult>("rnd_abort", { executionId }),

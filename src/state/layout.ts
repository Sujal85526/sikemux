...
// Insert `newPane` next to `paneId`. If the pane already sits inside a split
// of the requested direction, the pane is added as a sibling (the target
// pane gives up half its space); otherwise the leaf becomes a 2-way split.
...
// Remove a pane. Returns the new tree, or null if it was the only pane left.
...
// Root-to-leaf path of (split, childIndex) pairs leading to `paneId`.
...
// Geometry for every pane plus the draggable dividers between siblings.
...
// Geometric neighbour in a direction — the tmux M-h/j/k/l model.
...
// Adjust the deepest ancestor split on the relevant axis so the active pane
// grows toward `dir`, stealing space from the adjacent sibling.

...
import {
  rundeckApi,
  type LogEntry,
  type RundeckExecution as Execution,
  type RundeckStep,
...
const STEP_STATE: Record<
  string,
  { label: string; cls: "pending" | "running" | "ok" | "fail" | "skip" }
> = {
    ...
    <div className="rnd-exec">
      <header className="rnd-exec-head">
        ...
        <div className="rnd-exec-head-r">
          ...
          {status === "running" && (
            <button
              className="rnd-btn-sm rnd-btn-danger"
              disabled={aborting}
              onClick={abort}
            >
        ...
        <aside className="rnd-steps">
          ...
          <div className="rnd-steps-list">
            {(state?.steps ?? []).length === 0 && (
              <div className="rnd-empty muted compact">waiting for steps…</div>
            )}
            {(state?.steps ?? []).map((s, i) => (
              <StepRow
                key={i}
                idx={i}
                step={s}
                selected={stepFilter === String(i + 1)}
                onClick={() => setStepFilter(String(i + 1))}
              />
            ))}
          </div>
        </aside>
          ...
          <div ref={logRef} className="rnd-logs-stream">
            {filteredEntries.length === 0 && (
              <div className="rnd-empty muted compact">
                no output{stepFilter ? " for this step" : ""} yet
              </div>
            )}
            {filteredEntries.map((e, i) => (
              <div
                key={i}
                className={`rnd-log-line${
                  e.level ? ` lvl-${e.level.toLowerCase()}` : ""
                }`}
              >
                <span className="rnd-log-step">
                  {e.stepctx ? `[${e.stepctx}]` : ""}
                </span>

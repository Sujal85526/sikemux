...
export function GitCmdLogBar() {
  ...
  if (!open) {
    return (
      <button
        type="button"
        className="git-log-bar collapsed"
        onClick={toggleGitCmdLog}
        title="Show command log (@)"
      >
        <span className="kbd">@</span>
        <span className="git-log-bar-summary">
          {log.length === 0 ? "no commands yet" : `${log.length} cmds`}
        </span>
      </button>
    );
  }
  ...
  return (
    <div className="git-log-bar open">
      <div className="git-log-bar-head">
        ...
        <button
          type="button"
          className="git-log-bar-btn"
          onClick={clearGitCmdLog}
      ...
      <div className="git-log-bar-rows">
        {log.length === 0 && (
          <div className="git-log-bar-empty">
            log empty — every git command will appear here
          </div>
        )}
        {log
          .slice()
          ...
          .map((e) => (
            <div key={e.id} className={`git-log-row status-${e.status}`}>
              ...
              <span className="git-log-row-glyph">
                {e.status === "running"
                  ? "…"
                  : e.status === "ok"
                    ? "✓"
              ...
              {e.detail && (
                <span className="git-log-row-detail">
                  {firstLine(e.detail)}
                </span>
              )}
            </div>
